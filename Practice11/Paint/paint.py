...
def main():
    ...
    radius = 15
    mode = 'draw'        # mode: draw, rect, circle, eraser
    points = []          # temporary list of points for the current line
    start_pos = None     # start position for rect and circle
    drawings = []        # list of all drawn shapes
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                if event.buttons[0]:
                    if mode == 'draw':
                        # Save line segment in drawings
                        points.append(event.pos)
                        if len(points) >= 2:
                            drawings.append(('line', current_color, points[-2], points[-1], radius))

                    elif mode == 'eraser':
                        # Eraser is saved as a big white circle in drawings
...
def drawLineBetween(screen, index, start, end, width, current_color):
    # Draw a line between two points using circles
    dx = start[0] - end[0]
    dy = start[1] - end[1]
